...
describe("style feature catalog", () => {
  it("creates disabled defaults for every configurable feature", () => {
    ...
    expect(profile.paragraphHover.values.backgroundColor).toBe("var(--style-editor-card-bg-soft)")
    ...
    expect(profile.editorBackground.values.backgroundColor).toBe("var(--style-editor-editor-bg)")
    expect(profile.headingSpacing.values.headingTopMargin).toBe(0.5)
    expect(profile.unorderedListMarkerColor.values.markerColor).toBe("var(--style-editor-list-marker)")
    expect(profile.markStyle.values.backgroundColor).toBe("var(--style-editor-mark-bg)")
    expect(profile.markStyle.values.emphasisThickness).toBe(2)
    expect(profile.inlineCodeStyle.values.color).toBe("var(--style-editor-inline-code-color)")
    expect(profile.inlineCodeStyle.values.radius).toBe(4)
    expect(profile.blockRefStyle.values.color).toBe("var(--style-editor-link-color)")
    ...
    expect(profile.listBulletLine.values.lineColor).toBe("var(--style-editor-list-line)")
    ...
    expect(profile.docTag.values.secondaryBg).toBe("var(--style-editor-tag-bg)")
    expect(profile.docTreeColorBlocks.values.color1).toBe("var(--style-editor-tree-block-1)")
